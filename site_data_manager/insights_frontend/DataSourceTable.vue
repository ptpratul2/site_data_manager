...
const tableName = decodeURIComponent(props.table_name)

tableStore.fetchTable(props.data_source, tableName).then((t) => {
	table.value = t
})

watchEffect(() => {
	document.title = `Tables | ${tableName}`
})
</script>
	...
	<header class="flex h-12 items-center justify-between border-b py-2.5 pl-5 pr-2">
		<Breadcrumbs
			:items="[
				...
				{
					label: tableName,
					route: `/data-source/${props.data_source}/${encodeURIComponent(tableName)}`,
				},
			]"
		/>
